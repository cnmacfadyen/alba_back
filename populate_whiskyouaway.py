...
def populate():
	...
	for item in events:
		r = add_events(item["name"], item["description"], get_categories(item["categories"]), item["image"], item["url"])
		print(format(str(r)))
...
def add_events(name, description, categories, image, url):
	r = Events.objects.get_or_create(name = name, description = description, categories = categories, image = image, url=url)
	return r
